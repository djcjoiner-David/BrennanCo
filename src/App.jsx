...
// Converts a "#RRGGBB" hex string to an "rgba(r,g,b,a)" string, so opacity-based
// hover/border states can reference BRAND_GOLD/BRAND_CREAM instead of a
// hardcoded DJC color literal.
...
function LoginScreen({onLogin}) {
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      sessionStorage.setItem("brennan_user",JSON.stringify({email:user.email,role:user.role,name:user.name,id:user.id}));
...
export default function DJCJoiner() {
  ...
  const [currentUser,setCurrentUser]=useState(()=>{
    try{const u=sessionStorage.getItem("brennan_user");return u?JSON.parse(u):null;}catch{return null;}
  });
  if (!currentUser) return <LoginScreen onLogin={setCurrentUser}/>;
  return <MainApp currentUser={currentUser} onLogout={()=>{sessionStorage.removeItem("brennan_user");setCurrentUser(null);}}/>;
}
      ...
      <div style={{background:BRAND_HEADER_BG,padding:"0 24px",position:"sticky",top:0,zIndex:100,boxShadow:"0 2px 8px rgba(0,0,0,0.15)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",paddingTop:14,paddingBottom:14}}>
          <div style={{display:"flex",alignItems:"center",gap:14}}>
            ...
            <div style={{fontSize:14,color:BRAND_CREAM,opacity:0.7}}>Production Schedule</div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{display:"flex",alignItems:"center",gap:8,background:"rgba(255,255,255,0.08)",borderRadius:8,padding:"6px 12px"}}>
              ...
              <div>
                <div style={{fontSize:13,color:BRAND_CREAM,fontWeight:500}}>{currentUser.name}</div>
            ...
            <button onClick={()=>setWorkHoursOpen(true)}
                ...
                <button onClick={onLogout} style={{padding:"7px 12px",borderRadius:8,fontSize:12,cursor:"pointer",border:"1px solid rgba(255,255,255,0.15)",background:"transparent",color:hexToRgba(BRAND_CREAM,0.6)}}>Sign Out</button>
          </div>
        </div>
        <div style={{display:"flex"}}>
          {[["schedule","📅 Schedule"],["summary","📋 Job Summary"]].map(([key,label])=>(
            <button key={key} onClick={()=>setTab(key)} style={{padding:"9px 22px",fontSize:14,fontWeight:500,cursor:"pointer",background:"none",border:"none",borderBottom:tab===key?`2.5px solid ${BRAND_GOLD}`:"2.5px solid transparent",color:tab===key?BRAND_GOLD:hexToRgba(BRAND_CREAM,0.55),transition:"all 0.15s"}}>{label}</button>
          ))}
        </div>
      </div>
